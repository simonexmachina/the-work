...
function extractFlatStatements(worksheet) {
  ...
  return sorted
    .map(idx => {
      ...
      if (!statement && !q1True && !q2Absolutely && !q3React && !q4Without && !turnaround) {
        return null;
      }
...
function getSituationPreview(worksheet) {
  ...
  let preview =
    previewBase.length > CHARACTER_LIMIT ? previewBase.slice(0, CHARACTER_LIMIT) : previewBase;
...
async function renderWorksheetsList() {
  ...
  try {
    ...
    listContainer.innerHTML =
      '<p class="text-center py-12 text-red-500">Error loading worksheets. Please refresh the page.</p>';
  }
}
...
async function loadWorksheetIntoForm(id) {
  ...
  const statementCount =
    Array.isArray(worksheet.statements) && worksheet.statements.length > 0
...
async function saveFormData() {
  ...
  statementGroups.forEach(group => {
    ...
    if (statementText || q1True || q2Absolutely || q3React || q4Without || turnaround) {
      statements.push({
